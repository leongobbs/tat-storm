...
// Запрос для получения количества данных по типам из imasparql
...
/**
 * Получает данные из imasparql для отображения на карточке
 * @returns Массив данных или undefined
 */
export async function fetchCardDetails(): Promise<CardDetail[] | undefined> {
  ...
  try {
    ...
    // Парсим JSON ответ в объект типа ImasparqlResponse
    const json: ImasparqlResponse = await res.json();

    // Преобразуем полученные данные в формат CardDetail и возвращаем
    return createCardDetails(json);
  } catch (err) {
    console.error(err); // Логируем ошибку, если произошел сбой
    return;
  }
}
